...
for index, row in df.iterrows():
    ...
    with open(f"../data/{file_name}_v2fixed_{index+1}.raw.txt", "w") as file:
        file.write(f"==========sqe==========\n\n{row['sqe']}\n\n==========急診去辨識病歷==========\n\n{row['急診去辨識病歷']}\n\n==========住院去辨識病歷==========\n\n{row['住院去辨識病歷']}\n\n==========檢驗紀錄==========\n\n{row['檢驗紀錄']}")


    # 從f"../data/{file_name}_v2fixed.{index+1}.raw.txt"讀取content
    content = ""
    with open(f"../data/{file_name}_v2fixed_{index+1}.raw.txt", "r") as file:
        content = file.read()
        # print("split_content:")
        # print(content)
    ...
    # 儲存標準化後的文本到f"../data/{file_name}_v2fixed_{index+1}.raw.standardized.txt"
    with open(f"../data/{file_name}_v2fixed_{index+1}.raw.standardized.txt", "w") as file:
    ...
    # 移动指定文件到输入目录
    source_file = f"../data/{file_name}_v2fixed_{index+1}.raw.standardized.txt"
    destination_file = os.path.join(input_dir, f"{file_name}_v2fixed_{index+1}.raw.standardized.txt")
    ...
    cTAKES_output_file = f"{output_dir}/{file_name}_v2fixed_{index+1}.raw.standardized.txt.xmi"
    ...
    # create a new file named f"../data/{file_name}_v2fixed_{index+1}.raw.standardized.cTAKES.txt" and write the content of source_file and cTAKES_output_file into it
    standardized_with_cTAKES_file = f"../data/{file_name}_v2fixed_{index+1}.raw.standardized.cTAKES.txt"
    ...
    # 儲存萃取出的結構化資訊到f"../data/{file_name}_v2fixed_{index+1}.raw.standardized.cTAKES.LLM.txt"
    with open(f"../data/{file_name}_v2fixed_{index+1}.raw.standardized.cTAKES.LLM.txt", "w") as file:
    ...
    # standardized_content handler
    completion = client.beta.chat.completions.parse(
        model="gpt-4o-2024-08-06",
        messages=[
            {"role": "system", "content": """
             You are an expert in structured medical data extraction, specializing in SNOMED_CT, RxNorm, and LOINC.
             Your task is to analyze standardized medical text.
             You should identify and extract the relevant LOINC, SNOMED_CT and RxNorm codes wherever applicable.
             Additionally, ensure that corresponding codes are accurately identified and supplemented.
             Convert the information into the given structured format:
            ...
            - `RxNorm`: A list of dictionaries, each containing the following keys:
                ...
                - `text`: The raw text you found in the original content.
            """},
            {"role": "user", "content": standardized_content}
        ],
        response_format=JSONStructure,
    )
    ...
    # 儲存萃取出的結構化資訊到f"../data/{file_name}_v2fixed_{index+1}.raw.standardized.LLM.txt"
    with open(f"../data/{file_name}_v2fixed_{index+1}.raw.standardized.LLM.txt", "w") as file:
        file.write(parsed_response.model_dump_json(indent=2))
    row_format_time = time.time()
    print(f"Row {index+1} format time: {row_format_time - row_standardized_time} seconds")
    row_end_time = time.time()
