...
preview_count = 5
# target setting
model1 = "mc_modelpack_snomed_int_16_mar_2022_25be3857ba34bdd5.zip"
model2 = "umls_self_train_model_pt2ch_3760d588371755d0.zip"
...
cat1 = CAT.load_model_pack(f'../models/{model1}')
cat2 = CAT.load_model_pack(f'../models/{model2}')
...
for index, row in df.iterrows():
    ...
    # 使用MedCAT從文本中提取實體
    # TODO: 一次一句
    entities1 = cat1.get_entities(standardized_content)
    entities2 = cat2.get_entities(standardized_content)
    # Save the entities to a JSON file
    with open(f"../data/pipe_result/{file_name}_{tool_name}_{version}_{row['sqe']}.raw.polishing.MedCAT1.json", "w") as json_file:
        json.dump(entities1, json_file, indent=2)
    with open(f"../data/pipe_result/{file_name}_{tool_name}_{version}_{row['sqe']}.raw.polishing.MedCAT2.json", "w") as json_file:
        json.dump(entities2, json_file, indent=2)
    linguistic_extraction_time = time.time()
    print(f"{Fore.GREEN}sqe {row['sqe']} linguistic extraction (model 1 & 2) time: {round(linguistic_extraction_time - preprocess_time, 2)} sec{Style.RESET_ALL}")

    # preparing voting info, like:
    # model_output_1 = [
    #     {'start': 0, 'end': 4, 'concept_id': 'C0012345', 'score': 0.9},
    #     {'start': 7, 'end': 10, 'concept_id': 'C0026789', 'score': 0.85}
    # ]
    # model_output_2 = [
    #     {'start': 0, 'end': 8, 'concept_id': 'C0012346', 'score': 0.95},
    #     {'start': 7, 'end': 13, 'concept_id': 'C0026790', 'score': 0.9}
    # ]
    model_output_list = []
