...
print("Opening 'MRCONSO.RRF'... (for creating 'filtered_umls.ndjson')")
with open('../data/dict/MRCONSO.RRF', 'r') as f:
    ...
    output_filename = f'../data/dict/filtered_umls{f"_{file_index}" if file_need_split else ""}.ndjson'
    ...
    for line in tqdm(lines, total=line_count, desc="Loading"):
        ...
        if columns[1] == 'ENG' and columns[11] in ['SNOMEDCT_US', 'LNC', 'RXNORM']:
            ...
            line_content = f'{{ "CUI": "{columns[0]}", "SAB": "{columns[11]}", "TTY": "{columns[12]}", "CODE": "{columns[13]}", "STR": "{columns[14]}" }}{NEW_LINE}'
            ...
            if file_need_split and (current_line_count == lines_per_file or file_size >= max_file_size):
                fo.close()  # 关闭当前文件
                file_index += 1  # 增加文件编号
                output_filename = f'../data/dict/filtered_umls_{file_index}.ndjson'
