...
for index, row in df.iterrows():
    ...
    with open(f"../data/{file_name}_v2_{index+1}.raw.txt", "w") as file:
        file.write(f"==========sqe==========\n\n{row['sqe']}\n\n==========急診去辨識病歷==========\n\n{row['急診去辨識病歷']}\n\n==========住院去辨識病歷==========\n\n{row['住院去辨識病歷']}\n\n==========檢驗紀錄==========\n\n{row['檢驗紀錄']}")


    # 從f"../data/{file_name}_v2_{index+1}.pipe.txt"讀取content
    content = ""
    with open(f"../data/{file_name}_v2_{index+1}.raw.txt", "r") as file:
        content = file.read()
        # print("split_content:")
        # print(content)
    ...
    # 儲存標準化後的文本到f"../data/{file_name}_v2_{index+1}.pipe.1.standardized.txt"
    with open(f"../data/{file_name}_v2_{index+1}.pipe.1.standardized.txt", "w") as file:
    ...
    # 儲存萃取出的結構化資訊到f"../data/{file_name}_v2_{index+1}.pipe.2.LLM.txt"
    with open(f"../data/{file_name}_v2_{index+1}.pipe.2.LLM.txt", "w") as file:
